...
class CheetahReaderApp {
    constructor(selector, options = {}) {
        ...
        setTimeout(() => {
            ...
            this._mobiService = new MOBIService(this._reader);
     ...
     */
    get mobiService() {
        console.warn('[DEPRECATED] Direct access to app.mobiService is deprecated. Use app.onMOBI() for events instead.');
        return this._mobiService;
    }
    ...
    loadMOBI(file) {
        if (this._mobiService) {
            this._mobiService.loadBook(file);
        } else {
            console.error('MOBIService not initialized yet');
        }
    }
    ...
    // ========================================
    // PUBLIC API - MOBI EVENT SYSTEM
    // ========================================

    /**
     * Subscribe to MOBI service events
     *
     * Available events:
     * - 'bookLoadStarted' - { filename }
     * - 'bookLoaded' - { filename, hasContent }
     * - 'metadataUpdated' - { title, author, publisher, language, ... }
     * - 'mobiError' - { code, message, details }
     *
     * @param {string} event - Event name
     * @param {Function} callback - Callback function
     * @returns {Function} Unsubscribe function
     */
    onMOBI(event, callback) {
        if (this._mobiService) {
            return this._mobiService.on(event, callback);
        }
        // Return no-op unsubscribe if service not ready
        return () => {};
    }
    ...
    /**
     * Unsubscribe from MOBI service events
     * @param {string} event - Event name
     * @param {Function} callback - Callback function
     */
    offMOBI(event, callback) {
        if (this._mobiService) {
            this._mobiService.off(event, callback);
        }
    }
