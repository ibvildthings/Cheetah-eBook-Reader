...
class Renderer {
    ...
    async renderContent(html, mode, bionic) {
        ...
        // IMPORTANT: Always wrap words so click-to-flow works in normal mode
        let processedHtml = html;
        if (mode === 'flow') {
            processedHtml = this._makeFlow(html, bionic);
        } else {
            // In normal mode: wrap words for click detection, but without flow styling
            processedHtml = this._wrapWords(html, bionic);
        }
    ...
    /**
     * Wrap words for click detection (normal mode)
     * Similar to _makeFlow but without flow-specific styling
     * @private
     */
    _wrapWords(html, useBionic) {
        const temp = document.createElement('div');
        temp.innerHTML = html;
        ...
        const wrap = text => text.replace(/(\S+)/g, word => {
            ...
            return parts.map(part => {
                ...
                const idx = wordIndex++;
                // Wrap in flow-word class for click detection, but no inactive class
                return `<span class="flow-word" data-word-index="${idx}">${content}</span>`;
            }).join('');
        });
