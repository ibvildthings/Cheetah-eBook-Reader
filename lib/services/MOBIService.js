...
class MOBIService {
     ...
     * Uses event delegation on document to catch clicks before word click handler
     * @private
     */
    _attachLinkHandlers() {
        ...
        // Attach to DOCUMENT with capture phase to ensure we run BEFORE
        // any other click handlers (like the word click handler)
        this._linkClickHandler = (e) => {
            // Find the closest <a> element (in case we clicked a child element)
            const link = e.target.closest('a[href^="#"]');

            if (!link) return; // Not a link click

            // Check if this link is inside the content area (only handle our MOBI links)
            if (!contentArea.contains(link)) return;

            const href = link.getAttribute('href');
            if (!href || !href.startsWith('#')) return;

            // CRITICAL: Stop event propagation IMMEDIATELY to prevent word click handler
            e.preventDefault();
            e.stopPropagation();
            ...
            if (targetElement) {
                ...
                setTimeout(() => {
                    // Find the first .flow-word element AFTER the target anchor in DOM tree
                    // This is more reliable than viewport calculations
                    let firstWordAfterAnchor = null;

                    // Start searching from the anchor element
                    let currentNode = targetElement;

                    // Helper function to find next .flow-word in DOM tree
                    const findNextFlowWord = (startNode) => {
                        // Try next sibling first
                        let node = startNode.nextSibling;
                        while (node) {
                            if (node.nodeType === Node.ELEMENT_NODE) {
                                // Check if this node is a .flow-word
                                if (node.classList && node.classList.contains('flow-word')) {
                                    return node;
                                }
                                // Check if it contains a .flow-word
                                const wordInside = node.querySelector('.flow-word');
                                if (wordInside) {
                                    return wordInside;
                                }
                            }
                        ...
                        // No sibling found, go up to parent and try its next sibling
                        if (startNode.parentElement && startNode.parentElement !== contentArea) {
                            return findNextFlowWord(startNode.parentElement);
                        }

                        return null;
                    };

                    firstWordAfterAnchor = findNextFlowWord(currentNode);

                    if (firstWordAfterAnchor) {
                        const wordIndex = parseInt(firstWordAfterAnchor.dataset.wordIndex, 10);
                        if (!isNaN(wordIndex) && this.reader && this.reader.state) {
                            console.log(`📍 Setting word index to ${wordIndex} (first word after anchor in DOM)`);
                            // Directly set the word index in the reader's state
                            this.reader.state.flow.currentWordIndex = wordIndex;
                        }
                    } else {
                        console.warn('❌ Could not find word after anchor');
                    }
        ...
        // Attach to DOCUMENT with capture phase - this runs BEFORE any other handlers
        document.addEventListener('click', this._linkClickHandler, true);

        // Count links for logging
        const linkCount = contentArea.querySelectorAll('a[href^="#"]').length;
        console.log(`📎 Event delegation handler attached to DOCUMENT for ${linkCount} internal links`);
    }
     ...
     */
    _cleanup() {
        // Remove link handler if attached (from document)
        if (this._linkClickHandler) {
            document.removeEventListener('click', this._linkClickHandler, true);
