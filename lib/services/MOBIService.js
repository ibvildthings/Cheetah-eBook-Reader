    ...
    _attachLinkHandlers() {
        ...
        this._linkClickHandler = (e) => {
            ...
            if (targetElement) {
                ...
                setTimeout(() => {
                    // Find the first .flow-word element AFTER the target anchor in document order
                    // Get all words in the document
                    const allWords = Array.from(contentArea.querySelectorAll('.flow-word'));

                    if (allWords.length === 0) {
                        console.warn('❌ No flow words found');
                        return;
                    }

                    console.log(`🔍 Searching through ${allWords.length} words for first word after anchor`);

                    // Find the first word that comes after the target element in document order
                    let firstWordAfterAnchor = null;
                    for (let i = 0; i < allWords.length; i++) {
                        const word = allWords[i];
                        // compareDocumentPosition returns a bitmask
                        // DOCUMENT_POSITION_FOLLOWING (4) means 'word' comes after 'targetElement'
                        const position = targetElement.compareDocumentPosition(word);
                        if (position & Node.DOCUMENT_POSITION_FOLLOWING) {
                            firstWordAfterAnchor = word;
                            console.log(`✅ Found first word after anchor at array index ${i} of ${allWords.length}`);
                            break;
                        }
                    }
                    ...
                    if (firstWordAfterAnchor) {
                        ...
                        if (!isNaN(wordIndex) && this.reader && this.reader.state) {
                            console.log(`📍 Setting word index to ${wordIndex} (first word after anchor in document order)`);
