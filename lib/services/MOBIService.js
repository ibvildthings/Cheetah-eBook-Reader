...
class MOBIService {
    ...
    _attachLinkHandlers() {
        ...
        if (this._linkClickHandler) {
            contentArea.removeEventListener('click', this._linkClickHandler, true);
        }

        // Use event delegation: attach ONE handler to the content area
        // This works even if links are created/replaced during rendering
        ...
        // Attach handler using capture phase to intercept before other handlers
        contentArea.addEventListener('click', this._linkClickHandler, true);

        // Count links for logging
        const linkCount = contentArea.querySelectorAll('a[href^="#"]').length;
        console.log(`📎 Event delegation handler attached for ${linkCount} internal links`);
    }
     ...
     */
    _cleanup() {
        // Remove link handler if attached
        if (this._linkClickHandler) {
            const contentArea = document.querySelector('.ebook-text-content');
            if (contentArea) {
                contentArea.removeEventListener('click', this._linkClickHandler, true);
